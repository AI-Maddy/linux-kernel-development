...
## Top-level layout# :material-sitemap: Kernel Architecture

> The source tree looks intimidating, but a handful of top-level directories account for almost everything you'll touch.
...
```mermaid
graph LR
    A[arch slash] -->|arch-specific hooks| K[kernel slash]
    D[drivers slash] -->|driver model APIs| K
    F[fs slash] -->|VFS interface| K
    M[mm slash] -->|allocators, paging| K
    N[net slash] -->|socket layer| K
```
...
!!! warning "Confusing VFS with a filesystem"
    The VFS is not a filesystem itself — it's the dispatch layer. The actual on-disk logic lives in `fs/ext4`, `fs/btrfs`, and similar subdirectories.
