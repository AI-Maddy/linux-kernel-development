...
!!! warning "Wrong CROSS_COMPILE prefix"
    A cross-build silently falling back to the host compiler produces a binary for the wrong architecture. Verify the toolchain prefix resolves to a real compiler before running `make`.
